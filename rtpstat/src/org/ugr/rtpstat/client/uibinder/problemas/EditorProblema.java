...
public class EditorProblema extends LayoutPanel {
	...
	public EditorProblema(final NotificadorGeneral notificadorGeneral) {
		...
		/*
		 * final HTML topref = notificacionTop; ocultarNotificacion = new
		 * Timer() {
		 * 
			...
			rtpstatService.addProblema(p, new AsyncCallback<Long>() {
				public void onSuccess(Long result) {
					if (result == Long.MIN_VALUE) {
						notificadorGeneral.warning("Ha ocurrido un error al intentar guardar el problema. Si el error persiste, <a tabindex=\"0\" class=\"gwt-Anchor\" target=\"_blank\" href=\"http://code.google.com/p/rtpstat/issues/entry\">¡Cuéntanoslo!</a> ");
					} else {
