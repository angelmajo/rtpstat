...
public class Calculo implements IsSerializable {
  ...
  public Calculo(TipoCalculo tipo, Float parametro) {
    ...
    if (!necesitaParametro()) {
      this.tipo = null;
      throw new IllegalArgumentException("Para crear un cálculo de tipo " + tipo
          + " NO es necesario un parametro");
    }
  ...
  public String toShortString() {
    ...
    default:
      throw new IllegalStateException(
          "Existe un cálculo sin manejar, pidale al administrador que resuelva este problema (Unknown state: "
              + this + ")");
    }
    return out + "(X)";
  }
   ...
   * "Percentiles (P1-P99)", "Función de distribución en x (F(x))", "Moda (Mo)",
   * "Rango (R)", "Rango Intercuartílico (RI)", "Varianza (Var)",
  ...
  public static Calculo valueOf(String itemText) {
    if (itemText.matches("Decil[0-9]")) {
      ...
      return new Calculo(TipoCalculo.MomentoCentral, Float.parseFloat(itemText.replace(
          "MomentoCentral", "")));
    } else if (itemText.matches("MomentoNoCentral[0-9]")) {
      return new Calculo(TipoCalculo.MomentoNoCentral, Float.parseFloat(itemText.replace(
          "MomentoNoCentral", "")));
    } else if (itemText.matches("FunciónDeDistribucion[-+]?[0-9]*\\.?[0-9]+")) {
      return new Calculo(TipoCalculo.FunciónDeDistribucion, Float.parseFloat(itemText.replace(
          "FunciónDeDistribucion", "")));
    } else {
